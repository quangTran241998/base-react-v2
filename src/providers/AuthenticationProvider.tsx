...
interface AuthenticationContextI {
  ...
  isLogged: boolean;
  user: UserInfos;
  login: (userInfos: UserInfos) => void;
  logout: () => void;
}
...
const AuthenticationProvider = ({ children }: { children: any }) => {
  ...
  //! Function
  const login = useCallback((userInfos: UserInfos) => {
    setIsLogging(true);
    setToken(userInfos.accessToken);
    setUser(userInfos);
    httpServices.attachTokenToHeader(token);
    httpServices.saveTokenStorage(userInfos.accessToken);
