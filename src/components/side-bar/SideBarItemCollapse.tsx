...
const SidebarItemCollapse = (props: Props) => {
  ...
  useEffect(() => {
    isActiveDad ? setIsOpen(true) : setIsOpen(false);
  }, [pathUrl]);
      ...
      <div className="content" style={isOpen ? { ...openSearchExtend } : { ...closeSearchExtend }}>
        <div className=" list-disc my-1">
          {children?.map((item, index) => {
            const pathCurrent = `/${location.pathname.split("/")[2]}`;
            const pathItem = `/${item.path.split("/")[2]}`;
            const isActiveRouteChild = location.pathname === item.path || pathCurrent === pathItem;
            return (
              <div
                key={index}
