...
const SidebarItem = (props: Props) => {
  ...
  return (
    <div
      ...
      className={`flex items-center p-3 cursor-pointer rounded-sm hover:bg-hover-nav-bar mb-1 ${
        isActive && "bg-gradient-main"
      }`}
    >
      {<Icon color={isActive ? "#333" : "white"} />}
      <p
        className={`ml-2 font-medium text-white text-[14px] ${
          isActive && "[&&]:text-[#333] [&&]:font-bold"
        }`}
      >
        {displayText}
      </p>
